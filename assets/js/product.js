...
function waitForCart() {
    return new Promise((resolve) => {
        if (window.cartComponent) {
            resolve(window.cartComponent);
        } else if (window.getCartInstance) {
            resolve(window.getCartInstance());
        } else {
            let attempts = 0;
            const checkCart = setInterval(() => {
                ...
                if (window.cartComponent) {
                    ...
                    console.error('CartComponent не знайдено після 50 спроб (5 секунд)');
                    clearInterval(checkCart);
                    resolve(null);
                }
            }, 100);
...
const basePath = window.location.pathname.includes('/product.html') 
    ? window.location.pathname.replace('/product.html', '') 
    : '';
const productsPath = basePath ? `${basePath}/assets/js/data/products.json` : 'assets/js/data/products.json';


fetch(productsPath)
    .then(res => {
        if (!res.ok) {
            console.error('Помилка завантаження:', res.status, res.statusText);
    ...
    .then(allProducts => {
        ...
        if (addToCartBtn) {
            addToCartBtn.addEventListener('click', async function() {
                ...
                const cart = await waitForCart();
                
                if (!cart) {
                    console.error('Не вдалося отримати CartComponent!');
                    alert('Помилка: кошик не ініціалізовано. Спробуйте оновити сторінку.');
                    return;
                }
            ...
            console.error('Кнопку "Додати до кошика" не знайдено!');
        }
        ...
        if (buyNowBtn) {
            ...
            console.error('Кнопку "Купити зараз" не знайдено!');
        }
