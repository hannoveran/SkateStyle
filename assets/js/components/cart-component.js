...
class CartComponent {
    ...
    addToCart(productOrId) {
        if (this.isProcessing) {
            console.log('Додавання до кошика...');
            return;
        }

        this.isProcessing = true;

        // Визначаємо: це id чи об'єкт продукту
        let product = null;
        let productId = null;
        ...
        if (typeof productOrId === "object") {
            ...
            productId = productOrId.toString();
            product = this.products.find(p => p.id.toString() === productId);
        }

        if (!product) {
            this.isProcessing = false;
            console.error("Товар не знайдено у CartComponent:", productOrId);
            return;
        }
